-- ════════════════════════════════════════════════════════════════
-- ИМПОРТ ТЕСТОВЫХ НОВОСТЕЙ В SUPABASE
-- ════════════════════════════════════════════════════════════════
-- Скопируйте ВСЁ и выполните в Supabase SQL Editor:
-- https://supabase.com/dashboard/project/eofneihisbhucxcydvac/sql/new
-- ════════════════════════════════════════════════════════════════

-- Удалить старые тестовые новости (если есть)
DELETE FROM public.news WHERE slug LIKE 'test-%' OR slug LIKE 'novye-%' OR slug LIKE 'zapusk-%';

-- Вставить 5 реальных новостей
...
-- Новость 1: Новые стандарты корпоративного управления
(
  'Новые стандарты корпоративного управления в Кыргызстане',
  'novye-standarty-korporativnogo-upravleniya-v-kyrgyzstane',
  'КФА представляет обновленный Кодекс корпоративного управления, разработанный с учетом международных лучших практик и особенностей национальной экономики.',
  '<p>Кыргызская Ассоциация Финансистов рада объявить о выпуске обновленного Кодекса корпоративного управления. Новый стандарт разработан с учетом международных лучших практик и адаптирован к особенностям национальной экономики.</p>
...
-- Новость 2: Программа сертификации директоров
(
  'Запуск программы сертификации независимых директоров',
  'zapusk-programmy-sertifikacii-nezavisimyh-direktorov',
  'КФА запускает уникальную программу обучения и сертификации для независимых директоров. Программа соответствует международным стандартам и адаптирована для кыргызского рынка.',
  '<p>Мы рады представить новую образовательную программу для подготовки квалифицированных независимых директоров.</p>
...
-- Новость 3: Конференция ESG
(
  'Международная конференция по ESG в Бишкеке',
  'mezhdunarodnaya-konferenciya-po-esg-v-bishkeke',
  '15-16 ноября в Бишкеке пройдет международная конференция, посвященная внедрению ESG-принципов в корпоративное управление.',
  '<p>КФА совместно с международными партнерами проводит конференцию, посвященную устойчивому развитию и ESG-повестке.</p>
...
-- Новость 4: Исследование практик
(
  'Результаты исследования «Практики корпоративного управления в КР»',
  'rezultaty-issledovaniya-praktiki-korporativnogo-upravleniya-v-kr',
  'КФА представляет результаты ежегодного исследования состояния корпоративного управления в компаниях Кыргызстана за 2024 год.',
  '<p>Завершено масштабное исследование практик корпоративного управления в 150 крупнейших компаниях страны.</p>
...
-- Новость 5: Курсы для CFO
(
  'Новые курсы для финансовых директоров',
  'novye-kursy-dlya-finansovyh-direktorov',
  'Образовательный центр КФА объявляет набор на специализированные курсы повышения квалификации для финансовых директоров.',
  '<p>Стартует новая образовательная программа для CFO и финансовых руководителей компаний.</p>
...
-- ════════════════════════════════════════════════════════════════
-- ГОТОВО!
-- ✅ Импортировано 5 реальных новостей из Laravel проекта
-- ✅ Все новости опубликованы (status = 'published')
-- ✅ Добавлены категории
-- ✅ Две новости помечены как избранные (featured)
--
