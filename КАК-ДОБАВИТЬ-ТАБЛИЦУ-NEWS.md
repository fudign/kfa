# 📋 Создание таблицы новостей в Supabase

## Проблема

Таблица `news` не существует в базе данных Supabase, поэтому NewsManager не может загрузить новости.

## Решение - Выполните SQL скрипт

### Шаг 1: Откройте Supabase Dashboard

1. Перейдите на https://supabase.com/dashboard
2. Войдите в аккаунт
3. Выберите ваш проект (kfa-production)

### Шаг 2: Откройте SQL Editor

1. В левом меню нажмите **SQL Editor**
2. Нажмите **New query** (или используйте Quick start query)

### Шаг 3: Выполните SQL скрипт

1. Откройте файл **`CREATE-NEWS-TABLE.sql`** в этой папке
2. Скопируйте весь код (Ctrl+A, Ctrl+C)
3. Вставьте в SQL Editor (Ctrl+V)
4. Нажмите **Run** или **F5**

### Шаг 4: Проверьте результат

После выполнения SQL вы увидите:

```
Test news data inserted successfully
```

И таблицу с 5 тестовыми новостями:

| id | title | status | featured |
|----|-------|--------|----------|
| 1 | Добро пожаловать... | published | ⭐ true |
| 2 | Как работать с редактором... | published | false |
| 3 | Черновик новости... | draft | false |
| 4 | Работа с изображениями... | published | false |
| 5 | Новости о развитии... | published | ⭐ true |

### Шаг 5: Проверьте интерфейс

1. Откройте https://kfa-website.vercel.app/dashboard/news
2. **Обязательно выйдите и войдите снова** (чтобы обновить токен)
3. Вы должны увидеть список новостей

---

## Что создаст SQL скрипт:

### 1. Таблица `public.news`
- **id** - уникальный идентификатор
- **title** - заголовок новости
- **slug** - URL-friendly название
- **content** - HTML содержимое
- **excerpt** - краткое описание
- **image** - URL изображения
- **status** - статус (draft/published/archived)
- **featured** - избранная новость
- **author_id** - автор (связь с profiles)
- **category** - категория
- **tags** - теги (массив)
- **published_at** - дата публикации
- **created_at, updated_at** - служебные даты

### 2. Индексы для производительности
- По slug (для быстрого поиска по URL)
- По status (для фильтрации)
- По published_at (для сортировки)
- По featured (для избранных)
- По author_id (для фильтра по автору)
- По category (для категорий)

### 3. RLS Политики безопасности
- Публичный доступ к опубликованным новостям
- Чтение всех новостей для пользователей с правом `content.view`
- Создание для пользователей с правом `content.create`
- Редактирование для пользователей с правом `content.edit`
- Удаление для пользователей с правом `content.delete`

### 4. Триггеры
- Автоматическое обновление `updated_at` при изменении

### 5. Тестовые данные
- 5 новостей для проверки работы системы
- 3 опубликованных новости (2 избранных)
- 1 черновик
- Разные категории

---

## Если возникли проблемы

### Ошибка: "Table already exists"
Значит таблица уже создана. Проверьте данные:

```sql
SELECT * FROM public.news ORDER BY created_at DESC;
```

### Ошибка с permissions
Убедитесь что у вас есть права:

```sql
SELECT email, role, permissions
FROM public.profiles
WHERE role = 'admin';
```

Должно быть: `'content.view', 'content.create', 'content.edit', 'content.delete'`

### Новости не отображаются в интерфейсе

1. **Проверьте данные в БД:**
   ```sql
   SELECT COUNT(*) FROM public.news;
   ```

2. **Проверьте RLS политики:**
   ```sql
   SELECT * FROM pg_policies WHERE tablename = 'news';
   ```

3. **Проверьте консоль браузера (F12)** на ошибки

4. **Обязательно перелогиньтесь** - выйдите и войдите снова

---

## После выполнения SQL

Откройте https://kfa-website.vercel.app/dashboard/news

Вы должны увидеть:

```
📰 Управление новостями

[+ Создать новость]  [🔍 Поиск]  [📊 Фильтры]

┌─────────────────────────────────────────┐
│ ⭐ Добро пожаловать в систему...        │
│ 🟢 Опубликовано | Общее                 │
│ [👁️ Просмотр] [✏️ Редактировать] [🗑️ Удалить] │
└─────────────────────────────────────────┘

┌─────────────────────────────────────────┐
│ Как работать с редактором новостей      │
│ 🟢 Опубликовано | Инструкции            │
│ [👁️ Просмотр] [✏️ Редактировать] [🗑️ Удалить] │
└─────────────────────────────────────────┘

... и так далее
```

---

## Контакты для помощи

Если что-то не работает:
1. Проверьте консоль браузера (F12 → Console)
2. Проверьте Network tab (F12 → Network) на ошибки API
3. Убедитесь что вы вошли как admin
4. Попробуйте очистить кэш браузера (Ctrl+Shift+R)

✅ **После выполнения всех шагов новости будут работать!**
