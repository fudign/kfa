...
```typescript
import { uploadFile, getPublicUrl, deleteFile, listFiles, downloadFile, createSignedUrl } from '@/lib/supabase';
...
const {
  data: { signedUrl },
} = await createSignedUrl('private/doc.pdf', 3600);
```
...
// Update
const { data, error } = await supabase.from('users').update({ status: 'inactive' }).eq('id', userId);
...
supabase
  ...
  .on('postgres_changes', { event: '*', schema: 'public', table: 'users' }, (payload) => console.log('Change:', payload))
  .subscribe();
```
