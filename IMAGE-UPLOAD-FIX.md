# 🖼️ Исправление Загрузки Изображений - Решение

**Дата**: 2025-11-13
**Проблема**: Не работает и не отображается изображение при загрузке в новости
**Статус**: ✅ ИСПРАВЛЕНО

---

## 🔍 Найденные Проблемы

### 1. ❌ Недостающие Permissions

**Проблема**: API требует permissions, которые не были добавлены в базу:
```
media.view   - для просмотра медиафайлов  ❌ (отсутствовал)
media.upload - для загрузки файлов        ❌ (отсутствовал)
media.delete - для удаления файлов        ❌ (отсутствовал)
```

**Решение**: Создан seeder `AddMediaPermissionsSeeder.php`
- ✅ Добавлены permissions в базу
- ✅ Назначены роли admin
- ✅ Seeder уже выполнен

### 2. ❌ Жестко Закодированный Диск в MediaService

**Проблема**:
```php
// MediaService.php - старый код
$fullPath = $file->storeAs($path, $filename, 'public');  // ❌ Всегда 'public'
'disk' => 'public',  // ❌ Игнорирует .env
```

Но в `.env`:
```
FILESYSTEM_DISK=supabase  # ❌ Игнорировалось
```

**Решение**: Исправлен MediaService
```php
// Теперь использует конфиг из .env
$disk = config('filesystems.default');  // ✅ 'supabase' из .env
$fullPath = $file->storeAs($path, $filename, $disk);  // ✅ Использует правильный диск
```

### 3. ❌ Старый JWT Токен

**Проблема**: Токен пользователя не содержит новые permissions

**Решение**: Нужно перелогиниться!

---

## ✅ Что Было Исправлено

### 1. MediaService.php
- ✅ Использует `config('filesystems.default')` вместо жестко закодированного 'public'
- ✅ Разные пути для разных дисков:
  - **Supabase**: `2025/01/13/file.jpg` (без префикса 'media/', т.к. bucket уже называется 'media')
  - **Local/Public**: `media/2025/01/13/file.jpg` (с префиксом 'media/')
- ✅ Сохраняет правильный disk в базе данных

### 2. Permissions
- ✅ `media.view` - добавлен и назначен admin
- ✅ `media.upload` - добавлен и назначен admin
- ✅ `media.delete` - добавлен и назначен admin

### 3. API Routes
```php
// routes/api.php
Route::middleware(['auth:sanctum', 'permission:media.view'])->group(function () {
    Route::get('/media', [MediaController::class, 'index']);        // ✅ Работает
    Route::get('/media/{media}', [MediaController::class, 'show']);  // ✅ Работает
});

Route::middleware(['auth:sanctum', 'permission:media.upload'])->group(function () {
    Route::post('/media', [MediaController::class, 'store']);  // ✅ Работает
});
```

---

## 🚀 Как Использовать

### Шаг 1: Обновить JWT Токен (ОБЯЗАТЕЛЬНО!)

**Откройте эту ссылку:**
```
http://localhost:3000/auth/force-logout
```

Это автоматически:
1. Очистит старый токен
2. Перенаправит на страницу входа
3. После входа вы получите новый токен с `media.upload` permission

### Шаг 2: Войти в Систему

```
Email: admin@kfa.kg
Password: (из .env файла backend)
```

### Шаг 3: Создать Новость с Изображением

1. Откройте `/dashboard/news`
2. Нажмите **"Добавить новость"**
3. Заполните форму:
   - Заголовок: "Тестовая новость"
   - Контент: "Это тест загрузки изображения"
4. **Загрузите изображение:**

   **Вариант A: Через MediaPicker (Рекомендуется)**
   - Нажмите кнопку **"Выбрать"** рядом с полем "Изображение"
   - Откроется MediaPicker
   - Нажмите **"Загрузить"**
   - Выберите файл (JPG, PNG, WebP, GIF до 5MB)
   - Файл загрузится в Supabase Storage
   - Выберите загруженный файл
   - Нажмите **"Выбрать (1)"**
   - Изображение появится в превью! ✅

   **Вариант B: Вставить URL вручную**
   - Вставьте URL изображения в поле
   - Например: `https://example.com/image.jpg`
   - Превью появится автоматически

5. Нажмите **"Сохранить"**

---

## 🧪 Проверка Работоспособности

### 1. Проверить Permissions

```bash
cd kfa-backend/kfa-api
php artisan tinker
```

```php
// Проверить что permissions существуют
DB::table('permissions')->whereIn('name', [
    'media.view', 'media.upload', 'media.delete'
])->get(['name']);

// Должно вернуть 3 записи

// Проверить что admin имеет эти права
$admin = DB::table('roles')->where('name', 'admin')->first();
$count = DB::table('role_has_permissions')
    ->where('role_id', $admin->id)
    ->count();
// Должно быть 20+
```

### 2. Проверить API Напрямую

```bash
# Получить список медиа (требует auth + media.view)
curl -H "Authorization: Bearer YOUR_TOKEN" \
  http://127.0.0.1:8000/api/media

# Загрузить файл (требует auth + media.upload)
curl -X POST \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -F "file=@image.jpg" \
  -F "collection=news" \
  http://127.0.0.1:8000/api/media
```

### 3. Проверить Supabase Storage

1. Откройте Supabase Dashboard
2. Перейдите в **Storage** → **media** bucket
3. После загрузки файла через API, вы должны увидеть:
   ```
   media/
     2025/
       01/
         13/
           [random-hash].jpg  ✅
   ```

---

## 📁 Структура Хранения

### Supabase Storage (Продакшн)

```
Bucket: media
└── 2025/
    └── 01/
        └── 13/
            ├── abc123...xyz.jpg  ← Оригинал
            ├── def456...uvw.png  ← Оригинал
            └── ...
```

**URL формат:**
```
https://eofneihisbhucxcydvac.supabase.co/storage/v1/object/public/media/2025/01/13/abc123.jpg
```

### Local Storage (Разработка)

```
storage/app/public/
└── media/
    └── 2025/
        └── 01/
            └── 13/
                ├── abc123...xyz.jpg
                └── ...
```

**URL формат:**
```
http://localhost:8000/storage/media/2025/01/13/abc123.jpg
```

---

## ⚙️ Конфигурация

### Backend (.env)

```env
# Supabase Storage (для продакшна и локальной разработки)
FILESYSTEM_DISK=supabase
SUPABASE_URL=https://eofneihisbhucxcydvac.supabase.co
SUPABASE_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
SUPABASE_STORAGE_BUCKET=media
```

**Для локальной разработки без Supabase:**
```env
# Переключиться на local storage
FILESYSTEM_DISK=public

# И выполнить:
# php artisan storage:link
```

### Frontend (services/api.ts)

```typescript
// Media API - уже настроен! ✅
export const mediaAPI = {
  getAll: async (params) => {
    const response = await api.get('/media', { params });
    return response.data;
  },

  upload: async (file: File, collection?: string) => {
    const formData = new FormData();
    formData.append('file', file);
    if (collection) {
      formData.append('collection', collection);
    }

    const response = await api.post('/media', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
    return response.data;
  },
};
```

---

## 🔧 Troubleshooting

### Проблема: "У вас нет прав для просмотра медиа-библиотеки"

**Причина**: Старый JWT токен
**Решение**:
```
http://localhost:3000/auth/force-logout
```

### Проблема: "Ошибка при загрузке файлов"

**Причина 1**: Неправильный Supabase credentials
**Решение**: Проверьте `.env`:
```bash
cd kfa-backend/kfa-api
cat .env | grep SUPABASE
```

**Причина 2**: CORS ошибка в Supabase
**Решение**:
1. Откройте Supabase Dashboard
2. Storage → Configuration → CORS
3. Добавьте:
   ```
   http://localhost:3000
   http://localhost:8000
   https://your-production-domain.com
   ```

**Причина 3**: Bucket не существует
**Решение**:
1. Supabase Dashboard → Storage
2. Create bucket: `media`
3. Public: ✅ Yes
4. File size limit: 5MB

### Проблема: Файл загружается, но не отображается

**Причина**: URL генерируется неправильно
**Решение**: Проверьте Media model accessor:
```php
// app/Models/Media.php
public function getUrlAttribute(): string
{
    return Storage::disk($this->disk)->url($this->path);  // ✅
}
```

### Проблема: Backend ошибка "Class 'Storage' not found"

**Причина**: Не установлен Supabase Flysystem adapter
**Решение**:
```bash
cd kfa-backend/kfa-api
composer require quix-labs/laravel-supabase-flysystem
```

---

## 📊 Проверка После Исправления

### ✅ Что Должно Работать:

1. ✅ **MediaPicker открывается**
   - Кнопка "Выбрать" рядом с полем изображения
   - Модальное окно с галереей

2. ✅ **Загрузка файлов**
   - Drag & Drop работает
   - Кнопка "Загрузить" работает
   - Файлы появляются в Supabase Storage

3. ✅ **Отображение изображений**
   - Превью в MediaPicker
   - Превью в форме новости
   - Изображение в списке новостей
   - Изображение на публичной странице новости

4. ✅ **API доступен**
   - `GET /api/media` - список файлов
   - `POST /api/media` - загрузка файла
   - `DELETE /api/media/{id}` - удаление

---

## 🎯 Итог

### Что Было Сделано:

1. ✅ Добавлены недостающие permissions (`media.view`, `media.upload`, `media.delete`)
2. ✅ Исправлен MediaService для использования конфигурируемого диска
3. ✅ Разные пути для Supabase и локального хранилища
4. ✅ Media model accessor работает с любым диском
5. ✅ API endpoints защищены правильными permissions

### Что Нужно Сделать Пользователю:

1. ✅ Перелогиниться: `http://localhost:3000/auth/force-logout`
2. ✅ Войти с admin credentials
3. ✅ Открыть `/dashboard/news`
4. ✅ Создать новость с изображением
5. ✅ Радоваться! 🎉

---

## 🚀 Дополнительные Возможности

### Управление Медиафайлами

Откройте `/dashboard/media` для:
- 📁 Просмотра всех загруженных файлов
- 🔍 Поиска по названию
- 🏷️ Фильтрации по типу (изображения, документы)
- 🗑️ Удаления ненужных файлов
- ✏️ Редактирования alt-text и описания

### Типы Файлов

**Поддерживаемые форматы:**
```
✅ Изображения: JPG, JPEG, PNG, GIF, WebP, SVG
✅ Документы: PDF, DOC, DOCX
```

**Ограничения:**
```
Max размер: 5MB
Max ширина: без ограничений
Max высота: без ограничений
```

---

## 📝 Технические Детали

### Изменения в Коде

**1. MediaService.php** (строки 46-93)
```php
// До:
$fullPath = $file->storeAs($path, $filename, 'public');  ❌
'disk' => 'public',  ❌

// После:
$disk = config('filesystems.default');  ✅
$path = ($disk === 'supabase') ? date('Y/m/d') : 'media/' . date('Y/m/d');  ✅
$fullPath = $file->storeAs($path, $filename, $disk);  ✅
'disk' => $disk,  ✅
```

**2. AddMediaPermissionsSeeder.php** (новый файл)
```php
$newPermissions = [
    ['name' => 'media.view', ...],
    ['name' => 'media.upload', ...],
    ['name' => 'media.delete', ...],
];
```

**3. routes/api.php** (уже было)
```php
Route::middleware(['auth:sanctum', 'permission:media.view'])->group(function () {
    Route::get('/media', [MediaController::class, 'index']);  ✅
});

Route::middleware(['auth:sanctum', 'permission:media.upload'])->group(function () {
    Route::post('/media', [MediaController::class, 'store']);  ✅
});
```

---

*Разработано: 2025-11-13*
*Все исправлено! Перелогиньтесь и загружайте изображения!* 🚀📸
